...
def protenix2dock_task(self, score_args: dict):
    ...
    try:
        ...
        # The per-task engine container mounts the shared MSA cache at
        # /data/msa_cache (docker_cmd.protenix_runtime_mounts maps the
        # host's /data/boltz_msa_cache there), so the CLI path must use
        # the CONTAINER-side name.
