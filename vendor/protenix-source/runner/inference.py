...
def update_inference_configs(configs: Any, n_token: int) -> Any:
    ...
    # LMI4Boltz-style low-VRAM mode (opt-in via PROTENIX_LOW_VRAM env). Default is unchanged.
    low_vram = str(os.environ.get("PROTENIX_LOW_VRAM", "")).strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
    }
    # When low_vram is on, relax the protenix-v2 token wall so larger complexes can be
    # attempted (LMI4Boltz reaches >2660 tokens on 24GB); aggressive chunking compensates.
    token_wall = 3072 if low_vram else 2560
    if n_token > token_wall and configs.model_name in ["protenix-v2"]:
        raise AssertionError(
            f"protenix-v2 model does not support n_token > {token_wall}. It might cause OOM."
        )
